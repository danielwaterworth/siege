...
readCommand :: [String] -> Maybe (Ref -> DBOperation Ref (Reply))
    ...
    (let op = do
          ...
          case items of
            E.EOF ->
              E.yield [] E.EOF
            E.Chunks items' -> do
              let items''' = filter (globMatch pattern) $ map fst items'
              items'' <- op
              return $ items''' ++ items'' in
                op)
  return $ MultiReply $ Just $ map (BulkReply . Just) items)
...
writeCommand :: [String] -> Maybe (Ref -> DBOperation Ref (Reply, Ref))
...
writeCommand ["srem", key, field] = Just (\head -> do
  ...
  head' <- mapInsert head key ref'
  return (StatusReply "OK", head'))
writeCommand _ = Nothing

command c =
  case (readCommand c, writeCommand c) of
...
sendReply (ErrorReply e) = send $ "-" ++ e ++ "\r\n"
sendReply (StatusReply s) = send $ "+" ++ s ++ "\r\n"
sendReply (IntegerReply n) = send $ ":" ++ show n ++ "\r\n"
sendReply (BulkReply Nothing) = send "$-1\r\n"
sendReply (BulkReply (Just s)) = send $ "$" ++ (show $ length s) ++ "\r\n" ++ s ++ "\r\n"
sendReply (MultiReply (Nothing)) = send "*-1\r\n"
